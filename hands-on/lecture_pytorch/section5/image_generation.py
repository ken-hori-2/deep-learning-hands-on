...
for i in range(len(train_imgs)):
    for j in range(n_sample_in_img):
        ...
        input_data[sample_id] = train_imgs[i, j:j+n_time] # 現在までの行を入力とする
        correct_data[sample_id] = train_imgs[i, j+n_time] # 入力の次の行を正解とする
...
"テスト用のデータ" # 10個の画像を使って上半分から下半分を生成して表示
...
def generate_images():
    ...
    # 下半分をRNNにより生成した画像
    print("Generated:")
    gen_imgs = disp_imgs.clone() # 最初は元の画像をクローンしたものを使う
    plt.figure(figsize=(20, 2))
    for i in range(n_disp):
        for j in range(n_sample_in_img):
            x = gen_imgs[i, j:j+n_time].reshape(1, n_time, img_size) # 時系列データを範囲を指定して取り出す -> バッチサイズ, 時間, 画像サイズに変換
            # x = x.cuda()  # GPU対応
            gen_imgs[i, j+n_time] = net(x)[0] # 順伝播をしてバッチから取り出す([0]=バッチのうちの一つの予測結果)
        ax = plt.subplot(1, n_disp, i+1)
