...
from datetime import datetime as dt
df.date = df.date.apply(lambda d: dt.strptime(str(d), "%Y/%m/%d")) # "%Y/%m/%d"))
print("df : ", df)
# plt.plot(df['date'], df['actions'])
# plt.show()
...
def evaluate(flag, model, data_provider, criterion):
    ...
    if flag=='test':
        ...
        # index = df["date"][-(36)-1:-(12):1] # 後ろ20この予測
        # index2 = df["date"][-(12)-1:-1:1] # 後ろ20この予測
        # print("index: ", index)
        # print("len pred: ", len(pred))
        # # plt.plot(index, pred.squeeze().cpu().detach().numpy(), label='pred') # true.detach())
        # plt.plot(index, pred.squeeze().cpu().detach().numpy()[-(36)-1:-(12):1], label='true') # , color="blue") # true.detach())
        # plt.plot(index2, pred.squeeze().cpu().detach().numpy()[-(12)-1:-1:1], label='pred', color="green") # true.detach())
        " or "
        "main.py ver."
        all = df["date"][-(36)-1:-1:1]
        # 23ってなんの値？
        plt.plot(all, df['actions'][-(36 +23 -10)-1:-(1 +23 -10):1], label='true_data', alpha=0.5) # -60 ~ -1
        # plt.plot(df['date'], df['actions'], label='true', alpha=0.5)
        ...
        "main__.py ver."
        # # index_tgt = df["date"][-(12 -2)-1:-1:1] # target
        # # plt.plot(index_tgt, tgt.squeeze().cpu().detach().numpy()[-(12 )-1:-2:1], label='tgt') # , color="green") # true.detach())
        # index_tgt = df["date"][-(12 -1)-1:-1:1] # target
        # plt.plot(index_tgt, true.squeeze().cpu().detach().numpy()[-(12 )-1:-2:1], label='tgt(true)') # , color="green") # true.detach())
        
        # # index = df["date"][-(36+12):49+25] # -1:1] # -37 ~ -11
        # # index2 = df["date"][-(36+12):48+25] # -1:1] # -11 ~ の予測
        # # plt.plot(index, true.squeeze().cpu().detach().numpy(), label='true') # , color="blue") # true.detach())
        # # plt.plot(index2, pred.squeeze().cpu().detach().numpy(), label='pred', color="green") # true.detach())
        "***"

        
        # index = df["date"][36-30:36+data_len-30:1] # -(36 )-1:-(12 ):1] # 後ろ20この予測
        # index2 = df["date"][-(12 )-1:-(1 ):1] # 後ろ20この予測
        # plt.plot(index, pred.squeeze().cpu().detach().numpy()[9+30:36+data_len+30:1], label='true') # , color="blue") # true.detach())
        # # plt.plot(index2, pred.squeeze().cpu().detach().numpy()[-(12 )-1:-(1 ):1], label='pred', color="green") # true.detach())
        plt.legend()
        plt.grid(True)
